...
public class LoadyFourPhaseAnimation {
    ...
    private func createFourPhaseButton(){
        UIView.animate(withDuration: 0.3) {
            if case .normal((_, _ , _)) = self.currentPhase {
                self.loady.titleEdgeInsets = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
                (self.loady.layer.sublayers?.first(where: { $0.accessibilityHint == LayerTempKeys.tempLayer.rawValue}) as? CAShapeLayer)?.fillColor = UIColor.clear.cgColor
            }
            else {
                self.loady.titleEdgeInsets = UIEdgeInsets(top: 0, left: 28, bottom: 0, right: 0)
                (self.loady.layer.sublayers?.first(where: { $0.accessibilityHint == LayerTempKeys.tempLayer.rawValue}) as? CAShapeLayer)?.fillColor = UIColor.white.cgColor
            }
